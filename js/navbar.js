...
function loadNavbar() {
  ...
  const navHTML = `
    <!-- position:sticky keeps the navbar in normal document flow (unlike fixed),
         so there's no height to measure/guess and no spacer div needed. -->
    <div id="site-navbar" class="sticky top-0 left-0 w-full z-50">
      <nav id="nav-bar-inner" class="bg-jaggery flex items-center justify-between gap-4 px-6 md:px-10 py-4 transition-shadow duration-300" style="font-family:'Poppins', sans-serif;">
        ...
        <div class="flex items-center gap-4 md:gap-5 flex-shrink-0">
          ...
          <!-- Cart -->
          <a href="products.html" id="cart-link" aria-label="Cart" class="nav-icon-btn relative text-cream hover:text-marigold transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <style>
      ...
      #site-navbar .nav-icon-btn{ transition: transform .3s cubic-bezier(.34,1.56,.64,1), color .3s ease; }
      #site-navbar .nav-icon-btn:hover{ transform: translateY(-2px); }

      #site-navbar .nav-badge{
        position:absolute; top:-7px; right:-9px;
        background:#F2C14E; color:#3A2417; font-size:10px; font-weight:700;
  ...
  window.addEventListener("scroll", () => {
    navInner.classList.toggle("nav-scrolled", window.scrollY > 8);
  }, { passive: true });
  ...
  // Wishlist / Cart badge counts, read from localStorage so any page
  // (e.g. an "Add to cart" or heart button on a product card) can update
  // them by writing to these keys and firing "nicy:counts-updated".
  ...
  refreshBadges();
  window.addEventListener("storage", refreshBadges);        // updates from other tabs
  window.addEventListener("nicy:counts-updated", refreshBadges); // updates from this tab
}
