...
function updateUploadStatus() {
    ...
    if (total === 0) {
        ...
        imageUploadStatus.textContent = `${total} image(s) ready (max ${MAX_PRODUCT_IMAGES})`;
        imageUploadStatus.className = 'image-upload-status success';
    }
}
...
imageFilesInput.addEventListener('change', function () {
    const files = Array.from(this.files);
    if (files.length + getCurrentExistingImages().length > MAX_PRODUCT_IMAGES) {
        alert(`You can upload a maximum of ${MAX_PRODUCT_IMAGES} images.`);
        this.value = '';
        return;
    }
    renderImagePreviews(files, getCurrentExistingImages());
...
document.getElementById('productForm').addEventListener('submit', async (e) => {
    ...
    if (total > MAX_PRODUCT_IMAGES) {
        imageUploadStatus.textContent = `You have ${total} images, maximum is ${MAX_PRODUCT_IMAGES}.`;
        imageUploadStatus.className = 'image-upload-status error';
        return;
    }
