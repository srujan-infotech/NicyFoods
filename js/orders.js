            ...
            return `
                <tr id="row-${o._id}">
                    ...
                    <td>
                        <select
                            id="status-${o._id}"
                            class="status-select ${statusBadgeClass(o.status)}"
                            onchange="handleQuickStatusChange('${o._id}', this)"
                        >
...
async function handleQuickStatusChange(id, selectEl) {
    ...
    if (ok) {
        // repaint dropdown color to match new status
        STATUS_OPTIONS.forEach(s => selectEl.classList.remove(statusBadgeClass(s)));
        selectEl.classList.add(statusBadgeClass(newStatus));
        renderStatCountsAfterQuickChange();
    } else {
        // revert select back to server value on failure
        loadOrders();
    }
}
...
async function updateOrderStatus(id, newStatus, note = '') {
    try {
        const payload = { status: newStatus, note: note.trim() };
        const res = await apiFetch(`${ORDERS_API_BASE}/${id}`, {
            method: 'PUT',
            body: JSON.stringify(payload),
        });
...
// lightweight stat refresh without a full reload (keeps table scroll position)
function renderStatCountsAfterQuickChange() {
    const rows = document.querySelectorAll('#ordersTableBody select.status-select');
    let pending = 0, shipped = 0;
    rows.forEach(sel => {
        if (sel.value === 'pending') pending++;
        if (sel.value === 'shipped') shipped++;
    });
    document.getElementById('statPendingOrders').textContent = pending;
    document.getElementById('statShippedOrders').textContent = shipped;
}
...
window.openOrderDetail = openOrderDetail;
window.deleteOrder = deleteOrder;
window.handleQuickStatusChange = handleQuickStatusChange;
