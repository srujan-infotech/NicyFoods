...
async function loadNavCounts() {
    ...
    const [products, testimonials, orders, contacts, welcomeLeadsNew, companies, coreValues, qualityPromises, whyChooseUs] =
        await Promise.all([
            fetchCount('/api/products'),
            fetchCount('/api/testimonials'),
            fetchCount('/api/orders'),
    ...
    setBadge('navOrderCount', orders);
    setBadge('navContactCount', contacts);
    // Badge shows unactioned ("new") leads, so it works like an inbox counter
